...
class Grid implements Iterable<Cell> {
    ...
    int maxRows = 25;
    int cellSize = 35;
    Cell[][] cells = new Cell[maxCols][maxRows];
        ...
        for(int i = 0; i < cells.length; i++){
            for(int j = 0; j < cells[i].length; j++){
                switch (rand.nextInt(6)){
                    case 0:
                        cells[i][j] = new Road(colToLabel(i), j, 10+cellSize*i,10+cellSize*j);
                        break;
                    case 1:
                        cells[i][j] = new Water(colToLabel(i), j, 10+cellSize*i,10+cellSize*j);
                        break;
                    case 2:
                        cells[i][j] = new Mountain(colToLabel(i), j, 10+cellSize*i,10+cellSize*j);
                        break;
                    default:
                        cells[i][j] = new Grass(colToLabel(i), j, 10+cellSize*i,10+cellSize*j);
                        break;
                }
            }
        }
    ...
    public Cell randomCell(){
        ...
        int row = rand.nextInt(maxCols);
        int col = rand.nextInt(maxRows);
